...
NOTES:
------
This is an interesting, but it's quite complex, the problems I see at the moment are
The current problem I am trying to solve is that for models that use intermediate thermo
states, there is not a straightforward way of adding to them an analytical eos. The idea
would be to reuse the equation of state object used at the node level to generate
residual objects (sub-equations) which then are added to the main system, keeping
everything horizontal.

- How do I add intermediate equation arguments to the signature of the residual function
    - Super verbose, unstable, mulitple methods / dynamic method creation ~> hacky
    - How do I extra
"""
...
from adet.fluid.settings import FluidModel, AnalyticalFluidModel, ExternalFluidModel

from adet.tools.strings import get_arg_state, get_index
...
class CompositeEquation(EquationBase):
    ...
    def __init__(
        ...
        self._use_sub_equations = isinstance(fluid_model, AnalyticalFluidModel)
    ...
    def _build_sub_equations(self):
        ...
        ```
        """
        if not self._use_sub_equations:
            return
