...
def rectangular_vel_profile(kin_W0, kin_W1, oth_k_prof1):
    W_mean = (kin_W1 + kin_W0) / 2
    delta_W = safe_abs(kin_W1 - kin_W0)

    W_ss = W_mean + oth_k_prof1 * delta_W
    W_ps = W_mean - oth_k_prof1 * delta_W
...
#
class DentonRectProfInc(LossModel):
    """
    References
    ----------
...
class DentonTrapProfile(LossModel):
    """
...
class DentonRectProfile(LossModel):
    ...
    def residual(
        ...
        W_ss, W_ps = rectangular_vel_profile(W0, W1, k_prof1)

        # Static enthalpies
        stc_hmass_ss = h_rlt0 - W_ss**2 / 2
        stc_hmass_ps = h_rlt0 - W_ps**2 / 2
        ...
        # [Pa * m = N / m]
        delta_p = p_ps - p_ss
        pressure_integral = delta_p * camb_len1

        # Entropy generation from 2D viscous dissipation
        # [ kg / m^3 ] * [ m^3 / s^3 ] / [K] * [m]
        # = [ kg * m / s^3 / K ] = [ N / s / K ]
        entropy_integral_ps = (
            cd_prof1 * rho_ps * W_ps**3 / temp_ps * camb_len1
        )
        entropy_integral_ss = (
            cd_prof1 * rho_ss * W_ss**3 / temp_ss * camb_len1
        )
        ...
        # 1. Tangential momentum balance [N]
        delta_Vt = cs.fabs(Vt1 - Vt0)
        r1 = ch_mf1 * delta_Vt - pressure_integral * h1 * np.cos(
            stag1
        )
