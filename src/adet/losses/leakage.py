...
class DentonLeakageLoss(LossModel):
    ...
    def residual(
        ...
        dm = oth_dischCoeff1 * geo_tip_clearance1 * (2 * rho_ps * (p_ps - p_ss))
        leak_integral = trapezoid2(
            W_distr_ss**2 * (1 - W_distr_ps / W_distr_ss) * dm, xi_dimensional
        )

        return oth_delta_smass_leakage1 - geo_hh1 * leak_integral / (
