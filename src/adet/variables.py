...
class ThermoVariables(BaseEnum):
    ...
    Density = VarSpec(CoolProperties.Dmass.value, 'kg / m**3', 2.0, (1e-5, 800.0))
    Pressure = VarSpec(CoolProperties.Press.value, 'Pa', 5e5, (1, 150e5))
...
class KinematicVariables(VariableEnum):
    ...
    Omega = VarSpec('omega', 'rad / s', 0.5, (-20000.0, 20000.0), 0, True)
    V_mag = VarSpec('V', 'm / s', 0.5, (0.1, 700.0))
    V_mer = VarSpec('Vm', 'm / s', 0.5, (0.1, 700.0))
    V_tan = VarSpec('Vt', 'm / s', 0.5, (-700.0, 700.0))
    W_hub = VarSpec('W_hub', 'm / s', None, None, 0, True)
    W_mag = VarSpec('W', 'm / s', 0.5, (0.1, 700.0))
    W_mer = VarSpec('Wm', 'm / s', 0.5, (0.1, 700.0))
    W_tan = VarSpec('Wt', 'm / s', 0.5, (-700.0, 700.0))
    W_tip = VarSpec('W_tip', 'm / s', None, None, 0, True)
    MachPresh = VarSpec('mach_prsh', '', 2.0, (1.0, 10.0))
    MachThroat = VarSpec('mach_thr', '', 1.0, (0.1, 10.0))
...
class GeometricVariables(VariableEnum):
    ...
    SolidityMidspan = VarSpec('solidity_mid', '', 1.0, None, 0, True)
...
class OtherVariables(VariableEnum):
    ...
    CumMassFlow = VarSpec('cum_mf', 'kg / s', 20.0, (1e-7, 5e4), 0, True)
    ChanMassflow = VarSpec('ch_massflow', 'kg / s', 1.0, (1e-7, 5e4))
