...
class GeometricVariables(VariableEnum):
    ...
    FlareAngle = VarSpec('fl_angle', 'rad', 0.1, bounds=(-1.5, 1.5), scalar=True)
    ...
    MetalAngleHub = VarSpec('met_ang_hub', 'rad', -0.3, bounds=(-1.5, 1.5), scalar=True)
    MetalAngleTip = VarSpec('met_ang_tip', 'rad', -0.3, bounds=(-1.5, 1.5), scalar=True)
...
class OtherVariables(VariableEnum):
    ...
    StreamMassFlow = VarSpec('stream_mf', 'kg / s', 20.0, (0.0, 5e4))
    TotMassFlow = VarSpec('tot_mf', 'kg / s', 20.0, bounds=(0.0, 1e7), scalar=True)
    ChanMassflow = VarSpec('ch_massflow', 'kg / s', 1.0, (0.0, 5e4))
    ThrMassFlow = VarSpec('thr_mass', 'kg / s', 1.0)
    TargMassFlow = VarSpec('tgt_mass', 'kg / s', 1.0)
