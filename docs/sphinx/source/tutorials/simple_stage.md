...
:align: center
:width: 400px
Schematic of the node layout for the stage
```
...
Note that the node index and state of the variables declared for the updates is irrelevant. The same variables are used for updates across all states (total, static, relative total) and nodes.
...
Create the rotor by copying the stator and assigning the rotating shaft:
...
Assemble all components into a network:
...
### Step 6: Add System Equations

Add multi-component equations that define nondimensional coefficients and repeated stage conditions.
...
Explore how to customize loss models, add additional stages, or modify design constraints to perform parametric studies.
